"""
NFIP + NASA POWER Climate Markers + Event Type Classification (Filtered)
-----------------------------------------------------------------------
...
# ---------------------------------------
# STEP 2: Fetch NASA POWER climate markers
# ---------------------------------------
def get_nasa_power(lat, lon):
    """Return long-term annual averages (precip, wind, temp)."""
    try:
        url = (
            f"https://power.larc.nasa.gov/api/temporal/climatology/point?"
            f"parameters=PRECTOTCORR,WS10M,T2M&community=AG"
        ...
        return {
            "precip_mm": sum(data["PRECTOTCORR"].values()) / 12,  # mm/day
            "wind_mean_m_s": sum(data["WS10M"].values()) / 12,   # m/s
            "temp_mean_c": sum(data["T2M"].values()) / 12,       # °C
        }
    except Exception as e:
        print("NASA POWER failed:", e)
        return {"precip_mm": 0, "wind_mean_m_s": 0, "temp_mean_c": 0}
...
def main():
    ...
    print("\nFetching NASA POWER climate markers per state...")
    climate_rows = []
    for state, (lat, lon) in tqdm(STATE_COORDS.items()):
        climate_rows.append({"state": state, **get_nasa_power(lat, lon)})
    climate_df = pd.DataFrame(climate_rows)

    # Merge and add event type
    merged = df.merge(climate_df, on="state", how="left")
    ...
    print(f"\nSaved {len(merged)} NFIP records with climate + event type → {out_path}")
